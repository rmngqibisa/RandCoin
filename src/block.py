...
class Block:
    ...
    def mine(self, difficulty: int):
        ...
        # ⚡ Bolt Optimization: Use byte string formatting and hoist lookups
        # Escape any '%' characters in the payload to prevent format string vulnerabilities
        safe_suffix_bytes = suffix.encode().replace(b'%', b'%%')
        template_bytes = prefix.encode() + b'%d' + safe_suffix_bytes

        sha256 = hashlib.sha256

        while self.hash[:difficulty] != target:
            self.nonce += 1
            # Byte formatting is faster than string concatenation + encoding
            block_string = template_bytes % self.nonce
