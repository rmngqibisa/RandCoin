...
class Block:
    ...
    def mine(self, difficulty: int):
        ...
        # Fast path: Construct JSON string manually using byte string templating
        ...
        # Safe escape of % characters to prevent format string vulnerabilities
        suffix_b = suffix.encode().replace(b'%', b'%%')
        template = b'{"nonce": %d' + suffix_b

        # Hoist variables out of the loop for maximum performance
        sha256 = hashlib.sha256

        while self.hash[:difficulty] != target:
            self.nonce += 1
            # Byte templating avoids .encode() overhead and string concatenation per iteration
