...
class Block:
    ...
    def mine(self, difficulty: int):
        ...
        # ⚡ Bolt Optimization: Pre-encode suffix to bytes and use byte template to avoid `.encode()` per iteration
        suffix_bytes = (", " + json.dumps(static_content, separators=(', ', ': '))[1:]).encode().replace(b'%', b'%%')
        template = b'{"nonce": %d' + suffix_bytes

        # Hoist hashlib.sha256 lookup
        sha256 = hashlib.sha256

        while self.hash[:difficulty] != target:
            self.nonce += 1
            # Byte string formatting is faster than concatenation + encoding
