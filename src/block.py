...
class Block:
    ...
    def mine(self, difficulty: int):
        ...
        # ⚡ Bolt Optimization: Use byte templating and hoist lookups
        # Escape any '%' in suffix to prevent format string errors
        suffix_escaped = suffix.replace('%', '%%')
        template = (b'{"nonce": %d' + suffix_escaped.encode())
        sha256 = hashlib.sha256

        while self.hash[:difficulty] != target:
            self.nonce += 1
            # Byte templating is faster than string concatenation + encode()
