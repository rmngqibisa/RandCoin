...
class Block:
    ...
    def mine(self, difficulty: int):
        ...
        # ⚡ Bolt Optimization: Pre-compute byte template to avoid .encode() overhead in the loop
        suffix_bytes = suffix.encode().replace(b'%', b'%%')
        template = b'{"nonce": %d' + suffix_bytes

        # Hoist local variable lookup for performance
        sha256 = hashlib.sha256

        while self.hash[:difficulty] != target:
            self.nonce += 1
            # Byte string formatting is faster than string concatenation and encode()
